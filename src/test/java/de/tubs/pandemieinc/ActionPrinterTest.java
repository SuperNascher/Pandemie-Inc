...
/**
* Testing the ActionPrinter class.
* (Returns the given action as JSON String for the client)
* We use jackson to "parse" our JSON String as JSON Node
* to be independent of the String formatting.
*
...
class ActionPrinterTests {
	...
	void testPutUnderQuarantine() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.putUnderQuarantine(testCity, rounds));
	...
	void testCloseAirport() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.closeAirport(testCity, rounds));
	...
	void testCloseConnection() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.closeConnection(fromCity, toCity, rounds));
	...
	void testDevelopVaccine() throws JsonProcessingException {
		...
		Pathogen pathogen = new Pathogen("Dr. Evil", Strength.MIDDLE, Strength.MIDDLE,
			                             Strength.MIDDLE, Strength.MIDDLE);

		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.developVaccine(pathogen));
	...
	void testDeployVaccine() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.deployVaccine(pathogen, city));
	...
	void testDevelopMedication() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.developMedication(pathogen));
	...
	void testDeployMedication() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.deployMedication(pathogen, city));
	...
	void testExertInfluence() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.exertInfluence(testCity));
	...
	void testCallElections() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.callElections(testCity));
	...
	void testApplyHygienicMeasures() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.applyHygienicMeasures(testCity));
	...
	void testLaunchCampaign() throws JsonProcessingException {
		...
		// Parse the response
		JsonNode rootNode = this.objectMapper
			.readTree(ActionPrinter.launchCampaign(testCity));
